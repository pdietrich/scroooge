module AuthenticatedSystem
  protected
    ...
    def logged_in?
      # puts "logged_in? current_person = #{current_person.inspect}"
      current_person != :false
    end
    
    # Accesses the current person from the session.
    def current_person
      # puts 'current_person: @current_person = '+ @current_person.inspect
      @current_person ||= (session[:person] && Person.find_by_id_with_balance(session[:person])) || :false
      # puts '    @current_person = '+ @current_person.inspect
    ...
    def login_required
      ...
      # puts "username, passwd = #{username}, #{passwd}"
