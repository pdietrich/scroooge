...
#  end
  ...
  def test_should_not_rehash_password
    people(:quentin).update_attributes(:name => 'quentin2')
  ...
  # by pd
  def test_should_authenticate_person_with_email
    assert_equal people(:quentin), Person.authenticate(people(:quentin).email, 'test')
  end
  ...
  protected
    def create_person(options = {})
      Person.create({ :name => 'quire', :email => 'quire@example.com', :password => 'quire', :password_confirmation => 'quire' }.merge(options))
    end
end
